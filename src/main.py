...
class ChessApp(ctk.CTk):
    ...
    def switch_screen(self, name):
        ...
        if name == "home":
            screens.show_home(
                self.container,
                lambda: self.switch_screen("game"),
                lambda: self.switch_screen("history"),
            ).pack(fill="both", expand=True)
        elif name == "history":
            screens.show_history(
                self.container, lambda: self.switch_screen("home")
            ).pack(fill="both", expand=True)
        elif name == "game":
            self.setup_game_ui()
    ...
    def setup_game_ui(self):
        ...
        self.board_label = ctk.CTkLabel(
            self.container, text=self.logic.board.unicode(), font=("Courier New", 50)
        )
        self.board_label.pack(pady=30)

        self.entry = ctk.CTkEntry(
            self.container, placeholder_text="Move (e.g. e4)", width=250
        )
    ...
    def process_move(self):
        ...
        try:
            ...
            self.logic.move_history.append(san_move)
            self.logic.board.push(move)

            # Calculate current move number
            current_move_num = (len(self.logic.move_history) // 2) + 1

            # Update UI
            self.board_label.configure(text=self.logic.board.unicode())

            # Show score + Move counter
            score = self.logic.get_material_score()
            self.score_label.configure(text=f"Move {current_move_num} | {score}")

            self.entry.configure(border_color=["#979797", "#565b5e"])
